...
/* Return a pointer to a string containing the name of the symbol of
   ...
   code is not known.  */
const char *libiodefv2_strsource_sym(libiodefv2_error_t err)
{
  	libiodefv2_error_source_t source = libiodefv2_error_source(err);

  	if ( msgidxof(source) == msgidxof(LIBIODEFV2_ERROR_SOURCE_DIM) )
    		return NULL;
